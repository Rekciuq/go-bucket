...
import (
	...
	"github.com/Rekciuq/go-bucket/ent/migrate"
...
// UpdateOneID returns an update builder for the given id.
func (c *URLClient) UpdateOneID(id string) *URLUpdateOne {
...
// DeleteOneID returns a builder for deleting the given entity by its id.
func (c *URLClient) DeleteOneID(id string) *URLDeleteOne {
...
func (c *URLClient) Get(ctx context.Context, id string) (*Url, error) {
	return c.Query().Where(url.ID(id)).Only(ctx)
}

// GetX is like Get, but panics if an error occurs.
func (c *URLClient) GetX(ctx context.Context, id string) *Url {
	obj, err := c.Get(ctx, id)
	if err != nil {
		panic(err)
	}
